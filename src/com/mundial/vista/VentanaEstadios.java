...
import com.mundial.vista.componentes.ModalEstadio;
...
public class VentanaEstadios extends JPanel {
    ...
    private JPanel crearHeader() {
        ...
        JLabel lblSub = new JLabel("Administra los recintos deportivos donde se jugarán los partidos oficiales.");
        ...
        JButton btnAgregar = new JButton(" Añadir Estadio ");
    ...
    private JPanel crearBarraBusqueda() {
        ...
        JTextField txtBuscar = new JTextField("Buscar por ID, Nombre, Ciudad...");
        ...
        txtBuscar.addFocusListener(new java.awt.event.FocusAdapter() {
            public void focusGained(java.awt.event.FocusEvent e) {
                if (txtBuscar.getText().equals("Buscar por ID, Nombre, Ciudad...")) {
                    txtBuscar.setText("");
            ...
            public void focusLost(java.awt.event.FocusEvent e) {
                if (txtBuscar.getText().trim().isEmpty()) {
                    txtBuscar.setForeground(Color.GRAY);
                    txtBuscar.setText("Buscar por ID, Nombre, Ciudad...");
                }
            }
        });
        ...
        txtBuscar.addKeyListener(new KeyAdapter() {
            ...
            public void keyReleased(KeyEvent e) {
                ...
                if (text.equals("Buscar por ID, Nombre, Ciudad...")) return;
        ...
        btnLimpiar.addActionListener(e -> {
            ...
            txtBuscar.setText("Buscar por ID, Nombre, Ciudad...");
            sorter.setRowFilter(null);
            tablaEstadios.requestFocus(); // quitar foco al textfield
        });
    ...
    private JScrollPane crearTabla() {
        modeloTabla = new DefaultTableModel(new Object[]{"ID", "Nombre", "Ciudad", "Acciones"}, 0) {
            @Override
        ...
        tablaEstadios.setRowSorter(sorter);
        
        // Estilos premium para la tabla
        tablaEstadios.setRowHeight(40);
        tablaEstadios.setBackground(COLOR_PANEL);
        tablaEstadios.setForeground(Color.WHITE);
        tablaEstadios.setShowVerticalLines(false);
        tablaEstadios.setShowHorizontalLines(true);
        tablaEstadios.setGridColor(new Color(40, 50, 70));
        tablaEstadios.setSelectionBackground(new Color(212, 175, 55, 50));
        tablaEstadios.setSelectionForeground(Color.WHITE);
        tablaEstadios.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        
        tablaEstadios.getTableHeader().setFont(new Font("Segoe UI", Font.BOLD, 14));
        tablaEstadios.getTableHeader().setBackground(new Color(15, 20, 35));
        tablaEstadios.getTableHeader().setForeground(COLOR_ACENTO);
        tablaEstadios.getTableHeader().setPreferredSize(new Dimension(100, 40));
        tablaEstadios.getTableHeader().setBorder(BorderFactory.createMatteBorder(0, 0, 2, 0, COLOR_ACENTO));
        
        // Configurar botones de accion
        tablaEstadios.getColumnModel().getColumn(3).setCellRenderer(new TableActionCellRender());
        tablaEstadios.getColumnModel().getColumn(3).setCellEditor(new TableActionCellEditor(new TableActionCellEditor.TableActionListener() {
            @Override
        ...
        // Ajustar anchos
        tablaEstadios.getColumnModel().getColumn(0).setPreferredWidth(50);
        tablaEstadios.getColumnModel().getColumn(0).setMaxWidth(80);
        tablaEstadios.getColumnModel().getColumn(3).setPreferredWidth(90);
        tablaEstadios.getColumnModel().getColumn(3).setMaxWidth(90);
    ...
    private void cargarDatosTabla() {
        ...
        List<Estadio> lista = dao.listar();
        for (Estadio e : lista) {
            modeloTabla.addRow(new Object[]{e.getIdEstadio(), e.getNombreEstadio(), e.getCiudadEstadio(), ""});
        }
    }

    private void mostrarModalAgregar() {
        Window parent = SwingUtilities.getWindowAncestor(this);
        ModalEstadio modal = new ModalEstadio(parent, "Agregar Nuevo Estadio", null);
        modal.setVisible(true);
        
        if (modal.isConfirmado()) {
            Estadio nuevo = modal.getEstadio();
            if (dao.insertar(nuevo)) {
                JOptionPane.showMessageDialog(this, "Estadio agregado correctamente.");
                cargarDatosTabla();
            } else {
    ...
    private void mostrarModalEditar(int row) {
        int id = Integer.parseInt(tablaEstadios.getValueAt(row, 0).toString());
        String nombre = tablaEstadios.getValueAt(row, 1).toString();
        String ciudad = tablaEstadios.getValueAt(row, 2).toString();
        
        Estadio actual = new Estadio(id, nombre, ciudad);
        
        Window parent = SwingUtilities.getWindowAncestor(this);
        ModalEstadio modal = new ModalEstadio(parent, "Editar Estadio", actual);
        modal.setVisible(true);
        
        if (modal.isConfirmado()) {
            Estadio editado = modal.getEstadio();
    ...
    private void ejecutarEliminar(int row) {
        ...
        int confirm = JOptionPane.showConfirmDialog(this, 
            "¿Estás seguro de eliminar este estadio (ID: " + id + ")?", 
